...
function cryout_setting_mobile_fn() {
	...
	echo "</select>";
	$checkedClass = ($parabolas['parabola_hcontain']=='1') ? ' checkedClass' : '';
...
function cryout_setting_frontposts_fn() {
	...
	echo "<div><small>".__("Enable to display latest posts on the presentation page, below the columns. Sticky posts are always displayed and not counted.","parabola")."</small></div>";
} // cryout_setting_frontpage_fn()
...
function  cryout_setting_accentcolors_fn() {
	...
	echo "<div><small>".__("Accents #1 and #2 should either be the same as the link colours or be separate from all other colours on the site.<br />
     ...
     Accents #3 and #4 should be the lighter/darker than the content background colour, being used as borders/shades on elements where accent #5 is background colour.","parabola")."</small></div>";
}
...
foreach($items as $id=>$item) {
	...
	echo "</select>";
	echo "<div><small>".__("Hide all the post metas. All meta info and meta areas will be hidden.","parabola")."</small></div>";
}
